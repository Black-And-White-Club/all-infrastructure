...
| kubeseal --format=yaml --controller-name "${CONTROLLER_NAME:-sealed-secrets}" --controller-namespace "${CONTROLLER_NAMESPACE:-kube-system}" > "${SECRETS_REPO_DIR:-.}/sealed-sealed-secrets-backup-creds.yaml"
