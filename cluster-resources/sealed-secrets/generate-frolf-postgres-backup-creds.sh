...
| kubeseal --format=yaml --controller-name "${CONTROLLER_NAME:-sealed-secrets}" --controller-namespace "${CONTROLLER_NAMESPACE:-kube-system}" > "${SECRETS_REPO_DIR:-.}/sealed-frolf-postgres-backup-creds.yaml"

echo "Generated: ${SECRETS_REPO_DIR:-.}/sealed-frolf-postgres-backup-creds.yaml"
