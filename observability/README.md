...
1. Local node storage (recommended for small clusters):
   ...
   - Add `charts/grafana/values-local.yaml` (present) and use `observability/grafana-app-local.yaml` to deploy this variant.
   - The repo provides an example PV/PVC in `cluster-resources/pv-grafana-local.yaml` and `cluster-resources/pvc-grafana-local.yaml`. Update the `nodeAffinity` to the correct worker node and apply it to the cluster.
...
- Deploy the `grafana-local` app for local storage.
...
Note on safety: The sample `pv-grafana-local.yaml` uses `persistentVolumeReclaimPolicy: Retain`, which means deleting the PV resource will not delete data on disk; the PV will remain and the underlying files on the host are preserved. No manifest in this repo will automatically wipe or format the node's boot disk. Still, always check `nodeAffinity` and `path` before applying PV manifests.
...
- After verification and successful migration, delete the `grafana-local` app and any local PV/PVC if they are no longer needed.
